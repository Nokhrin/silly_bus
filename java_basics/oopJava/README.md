...
---
Отличия `абстрактных классов`, `интерфейсов`, `суперклассов`
- суперкласс - любой класс, от которого производится наследование
- Абстрактный класс, Интерфейс - подмножества суперкласса
- Абстрактный класс содержит поля и методы - то есть, состояние и поведение, общее для всех наследников
- Интерфейс содержит формальное описание состояние и поведения, ожидаемого от всех наследников

```mermaid
erDiagram
    SUPERCLASS ||--o{ ABSTRACT_CLASS : "наследует"
    SUPERCLASS ||--o{ INTERFACE : "реализует"
```
---
...
> Упрощение кода
> Один метод может работать с разными типами объектов, если они реализуют общий интерфейс или наследуют общий класс. 
> 
> Гибкость и расширяемость
> Новый код можно добавлять без изменения существующего. Например, добавить Cow — не нужно менять метод, который работает с Animal. 
> 
> Упрощение тестирования
> С помощью полиморфизма легко подменять реальные объекты моками (например, в TestNG). 
> 
> Снижение зависимости от конкретных реализаций
> Код работает с интерфейсами, а не с конкретными классами — это ключ к слабой связанности. 
> 
> Совместимость с паттернами проектирования
> Многие паттерны (Наблюдатель, Фабричный метод, Стратегия и т.д.) строятся на полиморфизме. 

- проблемы при отсутствии полиморфизма
> ❌ Сложность расширения
> 
> Добавление нового животного (например, Cow) требует изменения всех мест, где обрабатываются животные.
> 
> ❌ Увеличение сложности условий
> 
> if-else цепочки растут, нарушается принцип Open/Closed (открыт для расширения, закрыт для изменения).

- недостатки полиморфизма

**1. Сложность отладки** | Статический тип переменной — `Animal`, а динамический — `Dog`. При отладке сложно понять, **какой именно объект** вызывается.

```
Animal animal = new Dog();
animal.makeSound(); // Какой метод вызывается? Сначала смотрят в runtime!
```
| Трудно отследить, **какой именно класс** реализует метод. Ошибки в логике могут проявляться только в runtime

**2. Повышенная сложность понимания кода** | Код становится "скрытым" — поведение зависит от типа объекта в момент выполнения. | 
```
List<Animal> animals = Arrays.asList(new Dog(), new Cat());
for (Animal a : animals) {
    a.makeSound(); // Какой звук? В runtime — от объекта!
}
```
Начинающий программист может не понимать, **почему сработал один метод, а не другой**. |

**3. Риск ошибок в наследовании** | Если в подклассе переопределяется метод, но **неправильно** — поведение может сломаться. | 
```
class Dog extends Animal {
    @Override
    public void makeSound() {
        throw new RuntimeException("Собака не может лаять!"); // Ошибка!
    }
}
```

Появляется `RuntimeException` в runtime, который сложно отследить.
