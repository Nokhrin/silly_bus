...
### Команды, формат команд
- Аргументы разделяются **одиночными пробелами**
- Команда может иметь **любое количество аргументов**
- Ответы всегда начинаются с `<` и содержат:
    - `ok [сообщение]` - успех,
...
withdrawal <номер_счета> <сумма>    → снять со счета
...
- Тесты должны **покрывать все требования первого комплекта**: открытие, пополнение, снятие, баланс, закрытие
- Если возникают сложности с наследованием (например, при наследовании от `Map<Integer, Account>`), 
  - **можно временно оставить без наследования**, если это мешает тестированию.
