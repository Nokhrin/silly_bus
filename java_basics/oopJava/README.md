...
[реализация - рефлексия](src/main/java/NoOop/OpsQueueWithReflection.java)
[Oracle - пример рефлексии класса](https://docs.oracle.com/javase/tutorial/displayCode.html?code=https://docs.oracle.com/javase/tutorial/reflect/member/example/MethodParameterSpy.java)
...
Статическая типизация - определение типа сущности на этапе **компиляции**
- Java, C
Динамическая типизация - определение типа сущности на этапе **выполнения**
- Python, JS
...
#todo    
если сделать такой же код но на java
в частности определить только два класса A, B (не считая Main)
и попытаться использовать одну переменную
и прописать вызов hello()
то код вообще не скомпилируется

вопрос, почему ?
ответ нужен развернутый
