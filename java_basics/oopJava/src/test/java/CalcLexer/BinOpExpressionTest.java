...
@Test
public class BinOpExpressionTest {
    //region TestName: One Number Expression
    @Test(groups = {"expression"}, description = "Проверка вычисления одного числа: 1")
    public void testOneNumber() {
        Expression expr = new BinOpExpression(new NumValue(1.0), Parsers.Operation.ADD, new NumValue(0.0));
        // Используем ADD, чтобы использовать структуру, но результат должен быть 1.0
        // Альтернативно можно было бы сделать прямой вызов, но структура BinOpExpression позволяет только бинарные операции
    ...
    public void testOneNumberWith42() {
        Expression expr = new BinOpExpression(new NumValue(42.0), Parsers.Operation.ADD, new NumValue(0.0));
    ...
    public void testSimpleAddition() {
        Expression expr = new BinOpExpression(
                new NumValue(1.0),
    ...
    public void testAdditionChain() {
        // 1+2+3 = (1+2)+3 = 6
        Expression inner = new BinOpExpression(
                new NumValue(1.0),
                Parsers.Operation.ADD,
                new NumValue(2.0)
        );
