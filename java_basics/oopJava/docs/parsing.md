...
## Ход работы
парсинг выполняется в compile time
...
текст -> сканирование -> токены/лексемы -> парсинг -> абстрактное синтаксическое дерево / АСД -> анализ -> промежуточное представление
...
алгоритм, преобразующий последовательность лексем в грамматические конструкции по определенным правилам
правила определены в API языка программирования и для каждого ЯП уникальны
...
- источник ввода - консоль (ввод из файла, сокета не поддерживается)
...
### Концепты реализации
- обратное чтение - проход по структуре данных в порядке от последнего элемента к первому с целью построения иерархической структуры выражения
```
1 + 2 * 3
...
- стек - структура данных типа FIFO для временного хранения прочитанных лексем - для отслеживания парных элементов
```
1 + 2 * 3

шаг 1: стек: [3]
шаг 2: стек: [3, *]
шаг 3: стек: [3, *, 2] - может быть достаточно для вычисления
шаг 4: прочитан + => выражение на стеке завершено, вычисляем
шаг 5: стек: [6]
шаг 6: стек: [6, +]
шаг 7: стек: [6, +, 1] - завершенное выражение, вычисляем
шаг 8: стек: [7]
``` 
