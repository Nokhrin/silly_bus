...
(метод с перебором возможных правил применительно к одному и тому же выражению называется backtracking)
например,

```java
import java.util.Optional;

public static Optional<T> parseExpr(String src) {
    if (src.equals("+")) {
        return "add";
    } else if (src.equals("*")) {
        return "mul";
...
Результат возвращается следующей по стеку функции, вызвавшей текущую функцию

Для возможности РС требуется отсутствие левой рекурсии

левая рекурсия: A -> Aa  // A - в начале правой части - вызывается в первую очередь => бесконечный цикл

пример левой рекурсии: `expr -> expr + term`, то же в случае с "или": `expr -> term | expr + term`
в правой части - `expr + term` - слева та же функция, что в левой части - `expr`
такой вызов зациклится `expr` -> `expr` -> `expr` -> `expr` -> `expr` -> ...
...
A' -> + a A' | e  // правая рекурсия - A' - в конце правой части, вызывается в конце
e - пустая цепочка

> вопрос, на который ответ нашел не сразу: A' - везде в правой части, а значит -  всегда вызывается,  как может быть остановка?

A' - НЕ функция, это правило, как и e
применяется одно из правил
если применено e, то происходит возврат к предыдущему вызову на стеке

пример
дано правило `expr -> term | expr + term`
дано выражение `1 + 2`
даны методы parseExpr, parseTerm
