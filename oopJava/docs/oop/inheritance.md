...
- понятие `подтип` - `subtype`
  - частный случай типа
  - в терминах ООП подтип порождается в результате
    - реализации наследования
      - [пример подтипирования](../../src/main/java/DemoTyping/SubTypes.java)
    
    - реализации интерфейса
      - интерфейс - описание структуры класса - полей и методов без конкретной реализации
      - интерфейс - договоренность о том, какое поведение ожидается от объектов, реализующих это описание
      - в контексте типов - интерфейс - супертип, классы, реализующие интерфейс - подтипы
      - классы-подтипы соответствуют договоренностям, зафиксированным в интерфейсе, 
      - классы-подтипы могут быть использованы в вызовах, ожидающих интерфейс
      - [пример интерфейса](../../src/main/java/DemoTyping/Withdrawal.java)
...
- [проблема ромба при наследовании классов](/MultipleInheritance/Classes/Main.java)
...
[код - Переопределение метода](/MultipleInheritance/OperationOverriding.java)
...
[код - Перегрузка метода](/MultipleInheritance/OperationOverloading.java)
...
##### эксперимент
- [код - множественное наследование от интерфейсов с default методом](/MultipleInheritance/Interfaces/InterfaceDefaultCheck.java)
...
свойства
- совместимость с Java - компилируется в Java байт-код, который выполняет JVM
- позволяет использовать библиотеки Java, наследовать Java классы, реализовывать Java интерфейсы
- сочетает
  - статическую типизацию как в Java - проверка типов в compile
  - динамическую типизацию как в Python - проверка типов в runtime
    - `def x` обозначает: переменная `x` может ссылаться на объекты разных типов
    - изменение типа отслеживает механизм `динамического разрешения методов`
      - позволяет вызывать методы, которые не были определены на этапе компиляции
        - реализовано на механизме MethodMissing - рефлексия -  с помощью MetaClass
    - объект Groovy содержит MetaClass - множество метаданных о классе объекта
      - доступные методы, доступность методов
    - MetaClass можно изменить в runtime
```groovy
def x = "hello" // 
// def обозначает, что тип значения переменной определять на этапе runtime
println(x) // hello
