Цель
Дочитать до конца, понять, задать вопросы
...
11. Реализовать контейнер Или (Ethier)
    interface Ethier< А,В> {
    static < A,В> Ethier< А,В> left(A a, Class< В> b)
    static < A,В> Ethier< А,В> right (B b, Class< A > а)
    Optional< А> getLeft();
    Optional< В> getRight();
    }
