...
public class ASTBuilder extends CalculatorBaseVisitor<ExprValue> {
  ...
  private ExprValue applyAbs(ExprValue num) {
    return switch (num) {
      ...
      case ExprValue.DoubleValue doubleValue ->
          new ExprValue.DoubleValue(Math.abs(doubleValue.value()));
    };
  }
  ...
  public ExprValue applyBinOp(ExprValue left, ExprValue right, TerminalNode op) {
    ...
    double result =
        switch (op.getSymbol().getType()) {
          ...
          default ->
              throw new IllegalStateException("Неизвестный оператор: " + op.getSymbol().getType());
        };
