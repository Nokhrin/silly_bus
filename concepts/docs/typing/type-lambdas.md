...
[анонимный класс vs лямбда выражение](../../src/main/java/org/example/oop_modeling/AnonymAndLambda.java)  
...
### Пример: [функция факториала](../../src/main/java/org/example/oop_modeling/LambdasBasics.java#L87)  
...
[функциональный интерфейс - пример](../../src/main/java/org/example/oop_modeling/IIntegerAction.java)  
значение переменной - лямбда-выражение - обязано содержать параметры и возвращаемое значение типов, указанных в интерфейсе  
  
[Лямбда - базовые операции - пример](../../src/main/java/org/example/oop_modeling/LambdasBasics.java)  
  
  
### инициализация лямбда переменной значением на существующий метод  
  
`::` - создает ссылку сигнатуры метода на функциональный интерфейс  
метод, указанный справа от ::, находится в сущности слева от ::  
`::` как точка в `object.method` - обращение к методу, но которое будет выполнено при вызове метода функционального интерфейса  
  
`Class::staticMethod` - стат метод класса, без контекста  
...
[Лямбда <- существующий метод](../../src/main/java/org/example/oop_modeling/LambdasBasics.java#L77)  
  
`Class::instanceMethod` - метод экземпляра, контекст конкретного экземпляра, первый параметр: класс-получатель  

[Лямбда <- пример `String::length`](../../src/main/java/org/example/oop_modeling/LambdasBasics.java#L107)  
